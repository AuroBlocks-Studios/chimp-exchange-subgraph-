...
contract EventEmitter {
    ...
    event LogArgument(address indexed sender, bytes32 indexed identifier, bytes message, uint256 value, address indexed deployeraddress);

    // Define the event that will be emitted when an address is granted a permission
    event AuthorizationGranted(address indexed addr, bytes32 indexed identifier, address indexed deployeraddress);

    // Define the event that will be emitted when an address has a permission revoked
    event AuthorizationRevoked(address indexed addr, bytes32 indexed identifier, address indexed deployeraddress);

    // Define the event that will be emitted when an ownership is transferred
    event OwnershipTransferred(address indexed newOwner, address indexed deployeraddress);
    ...
    function emitEvent(bytes32 identifier, bytes memory message, uint256 value) public {
        ...
        emit LogArgument(msg.sender, identifier, message, value, address(0xD19f62b5A721747A04b969C90062CBb85D4aAaA8));
    }
    ...
    function authorize(bytes32 identifier, address addr) public {
        ...
        emit AuthorizationGranted(addr, identifier, address(0xD19f62b5A721747A04b969C90062CBb85D4aAaA8));
    }
    ...
    function removeAuthorization(bytes32 identifier, address addr) public {
        ...
        emit AuthorizationRevoked(addr, identifier,address(0xD19f62b5A721747A04b969C90062CBb85D4aAaA8));
    }
    ...
    function transferOwnership(address newOwner) public {
        ...
        emit OwnershipTransferred(newOwner,address(0xD19f62b5A721747A04b969C90062CBb85D4aAaA8));
    }
}
